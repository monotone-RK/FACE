...
int main(int argc, char *argv[]) {
  ...
  buf_pz  = (int*)malloc(sizeof(int)*DATANUM); // buffer for phase zero
  ...
  // phase 0 (can be used only in 2-tree)
  for (i = 0; i < (DATANUM/((ELEMS_PER_UNIT << WAY_LOG))); i++) {
    MergeSort(data, i*((ELEMS_PER_UNIT << WAY_LOG)), (i+1)*((ELEMS_PER_UNIT << WAY_LOG))-1);
    for (j = 0; j < (ELEMS_PER_UNIT << WAY_LOG); j++) {
      if      (i%P_NUM == 0) buf_pz[j+((i/2)*((ELEMS_PER_UNIT << WAY_LOG)))] = data[j+(i*((ELEMS_PER_UNIT << WAY_LOG)))];
      else if (i%P_NUM == 1) buf_pz[j+((i/2)*((ELEMS_PER_UNIT << WAY_LOG)))+(DATANUM/2)] = data[j+(i*((ELEMS_PER_UNIT << WAY_LOG)))];
    }
  }
  if (sim_type == 2) {
    printf("Phase 0 ==============================================================\n");
    for (i = 0; i < DATANUM; i++) {
      printf("%08x ", buf_pz[i]);
      if (i % (ELEMS_PER_UNIT << WAY_LOG) == (ELEMS_PER_UNIT << WAY_LOG)-1) printf("\n");
      if (i % (DATANUM/2) == (DATANUM/2)-1) printf("\n");
    }
  }
  for (i = 0; i < (DATANUM)/(32*16); i++) {
    for (j = 0; j < (32*16); j++) {
      if      (i%P_NUM == 0) data[j+(i*(32*16))] = buf_pz[j+((i/2)*(32*16))];
      else if (i%P_NUM == 1) data[j+(i*(32*16))] = buf_pz[j+((i/2)*(32*16))+(DATANUM/2)];
    }
  }
  
  
  // for (phase = 0; phase < LAST_PHASE+1; phase++) {
  // for (phase = 1; phase < LAST_PHASE+1; phase++) {
  for (phase = 1; phase < LAST_PHASE; phase++) {
    printf("Phase %d ==============================================================\n", phase);
    cbcnt  = 0;
