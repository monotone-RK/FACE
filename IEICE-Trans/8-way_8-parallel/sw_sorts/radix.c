...
int *radix;
int *temp;
void RadixSort(int sortdata[], int datanum, int radixmax) {
  int i, j;
  int index;
  int r_digit = 1;

  while (r_digit <= radixmax) {
    for (i = 0; i < datanum; i++) radix[i] = (sortdata[i]/r_digit) % 10;
    
    index = 0;
    for (i = 0; i < 10; i++) {
      for (j = 0; j < datanum; j++){
        if (radix[j] == i) temp[index++] = sortdata[j];
      }
    }

    for (i = 0; i < datanum; i++) sortdata[i] = temp[i];
