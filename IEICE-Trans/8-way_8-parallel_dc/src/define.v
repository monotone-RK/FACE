...
`define P_LOG                    3 // parallelism of merge sort tree (log scale)
`define IB_SIZE                  8 // input  buffer FIFO size, 6->64entry, 7->128entry ...
...
`define DRAM_RBLOCKS            64 // the number of blocks per DRAM read  access, 4 blocks = 2KB
                                    // this must be 2^n where n is a natural number
`define DRAM_WBLOCKS           128 // the number of blocks per DRAM write access, 8 blocks = 4KB
                                    // this must be 2^n where n is a natural number
`define REQ_THRE                26 // read request threthold, may be (1<<IB_SIZE - RBLOCKS - 1)
// `define REQ_THRE                 1 // read request threthold, may be (1<<IB_SIZE - RBLOCKS - 1)
...
/**************************************************************************************************/
// `define LAST_PHASE     1 
// `define DRAM_RBLOCKS   2
// `define DRAM_WBLOCKS   2
// `define LAST_PHASE     2 
// `define DRAM_RBLOCKS   8
// `define DRAM_WBLOCKS   8
