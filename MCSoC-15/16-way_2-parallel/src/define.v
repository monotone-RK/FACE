...
`define IB_SIZE                  6 // input  buffer FIFO size, 6->64entry, 7->128entry ...
`define OB_SIZE                  6 // output buffer FIFO size, 10->1024entry, 11->2048 entry ...  
`define DRAM_RBLOCKS            16 // the number of blocks per DRAM read  access, 4 blocks = 2KB
                                   // this must be 2^n where n is a natural number
`define DRAM_WBLOCKS            16 // the number of blocks per DRAM write access, 8 blocks = 4KB
                                   // this must be 2^n where n is a natural number
`define REQ_THRE                32 // read request threthold, may be (1<<IB_SIZE - RBLOCKS - 1)
